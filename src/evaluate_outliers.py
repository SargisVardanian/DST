#!/usr/bin/env python3
"""
Legacy exploratory evaluation path for saved rule models on:
- outliers/inliers subsets from previously saved outlier exports, and/or
...
- This script is not the canonical paper-facing analysis path anymore.
  The maintained benchmark/report flow now goes through:
  - build_report.py
...
def run_evaluation():
    parser = argparse.ArgumentParser(
        description="Legacy exploratory evaluation of saved DST models on outlier/inlier subsets."
    )
    ...
    parser.add_argument(
        ...
        help="What to evaluate: saved outlier/inlier subsets, full test split, or both.",
    )
    args = parser.parse_args()
    print(
        "[warn] evaluate_outliers.py is a legacy exploratory path. "
        "Use build_report.py + analyze_hard_cases.py for the maintained benchmark/report flow."
    )
    ...
    for ds_name in datasets:
        ...
        def eval_subset(*, subset_type: str, X_sub_df: pd.DataFrame, y_sub: np.ndarray, seed_offset: int) -> dict[str, dict]:
            ...
            for algo in algos:
                model_path = COMMON / "results" / "raw_runs" / "models" / f"{algo.lower()}_{ds_name}_dst.pkl"
