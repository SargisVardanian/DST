...
def filter_by_rule(
    ...
    if label_column not in df.columns:
        raise KeyError(f"'{label_column}' column is required")
    if "distance_norm" not in df.columns:
        raise KeyError("'distance_norm' column is required")
    if df["distance_norm"].isna().any():
        raise ValueError("distance_norm contains NaNs")
    if np.isinf(df["distance_norm"]).any():
        raise ValueError("distance_norm contains inf")
