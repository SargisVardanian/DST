...
try:
    ...
    from .core import (
        params_to_mass,
        ds_combine_pair,
        masses_to_pignistic as core_masses_to_pignistic,
        get_combined_rule as core_get_combined_rule,
        prepare_rules_for_export as core_prepare_rules_for_export,
    )
...
def _normalise_decoder_map(mapping: Optional[dict]) -> dict:
    """Expand decoder keys so int/float/string lookups all work after JSON/pickle loads."""
    out: dict = {}
    for raw_key, value in dict(mapping or {}).items():
        ...
        try:
            f_key = float(raw_key)
            out[f_key] = value
            if abs(f_key - round(f_key)) <= 1e-9:
                i_key = int(round(f_key))
                out[i_key] = value
                out[str(i_key)] = value
            out[str(raw_key)] = value
        except Exception:
            out[str(raw_key)] = value
    return out
...
class DSModelMultiQ(nn.Module):
    ...
    def _normalize_combination_rule(rule: Optional[str]) -> str:
        """Normalize combination rule string and reject unsupported values explicitly."""
        ...
        if r in {"dempster", "yager", "vote"}:
            return r
        raise ValueError(
            f"Unsupported combination_rule {rule!r}. Expected one of: dempster, yager, vote."
        )
    ...
    def _to_numpy(self, x, bool_cast: bool = False) -> np.ndarray:
        """Compatibility helper for shared explainer paths in core.py."""
        arr = _as_numpy(x)
        if bool_cast:
    ...
    def generate_rules(self, X, y=None, feature_names=None, *, algo=None, verbose=False, **params):
        ...
        if self.algo == "STATIC":
            static_param_names = {"breaks", "top_k_cats", "pair_top", "triple_top", "max_rules"}
            static_params = {key: value for key, value in params.items() if key in static_param_names}
            for r in gen.build_static_rules(
                X_raw,
            ...
            gen.fit(X_raw, y_np, feature_names=self.feature_names, value_decoders=self.value_names)
    ...
    def _condition_caption(self, cond):
        ...
        def _fmt(n, o, v):
            dec = _normalise_decoder_map(self.value_names.get(n) or {})
            d = dec.get(v, dec.get(str(v), v))
    ...
    def get_combined_rule(
        ...
        *,
        ...
        """Compatibility bridge to the shared combined-rule explainer in core.py."""
        if weight_key is not None and weight_key != getattr(self, "combination_weight_key", None):
            self.combination_weight_key = str(weight_key)
        return core_get_combined_rule(
            self,
            sample,
    ...
    def save_rules_dsb(self, path, *, decimals: int = 4, include_masses: bool = True):
        ...
        with Path(path).open("w", encoding="utf-8") as f:
            ...
            for i in order:
                r = self.rules[i]
                if include_masses and m is not None:
                    mass_row = np.round(np.asarray(m[i], dtype=float), int(decimals)).tolist()
                    f.write(f"class {r.get('label')} :: {r.get('caption')} || masses: {mass_row}\n")
                else:
                    f.write(f"class {r.get('label')} :: {r.get('caption')}\n")

    def save_rules_bin(self, path):
        """Save rules and params to binary file."""
        d = {"artifact_version": self.ARTIFACT_VERSION,
             "k": self.num_classes, "algo": self.algo, "feature_names": self.feature_names,
    ...
    def load_rules_bin(self, path):
        ...
        artifact_version = int(d.get("artifact_version", 0) or 0)
        if artifact_version != self.ARTIFACT_VERSION:
            raise ValueError(
                f"incompatible artifact_version {artifact_version}; expected {self.ARTIFACT_VERSION}"
            )
        ...
        loaded_value_names = d.get("value_decoders", self.value_names)
        self.value_names = {
            str(name): _normalise_decoder_map(mapping)
            for name, mapping in dict(loaded_value_names or {}).items()
        }
        raw_combination_rule = d.get("combination_rule", self.combination_rule or "dempster")
        self.combination_rule = self._normalize_combination_rule(raw_combination_rule)
        raw_rule_text = str(raw_combination_rule or "").strip().lower().replace("-", "_")
        if raw_rule_text and raw_rule_text != self.combination_rule:
            print(
                f"[warn] loading legacy combination_rule {raw_combination_rule!r} as {self.combination_rule!r}"
            )
    ...
    def prepare_rules_for_export(self, sample=None):
        """Compatibility bridge to the shared export helper in core.py."""
        if sample is None:
            ...
            if self.rule_mass_params is not None:
                out["masses"] = self._rule_masses().detach().cpu().numpy().tolist()
            return out
        return core_prepare_rules_for_export(self, sample)
