...
def cls_score(m, threshold=0):
    ...
    """
    return float(m[2])/(m[1] + m[2] + threshold + 1e-8)
...
def cls_pla_score(m, threshold=0):
    ...
    :return: Score of membership for the first class
    """
    return float(1 - m[2] - m[3])/(m[1] + m[2] + threshold + 1e-8)
...
def filter_duplicate_rules(rules):
    ...
    определяемые по текстовому описанию (caption). Если обнаружены дубликаты, оставляется первое вхождение.
    """
    unique = {}
    for rule in rules:
        key = rule.caption.strip().lower()
