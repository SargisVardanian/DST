...
from rule_generator import _condition_caption, _format_literal_value, _normalise_value
...
def build_merged_rule_explanation(*, selected_rules, beta: float = 1.0, eps: float = 1e-8, value_names: Optional[dict] = None):
    ...
    for row in selected_rules:
        ...
        for spec in list(row.get("specs") or []):
            if not isinstance(spec, (list, tuple)) or len(spec) != 3:
                continue
            feature, op, raw_value = str(spec[0]), str(spec[1]), _normalise_value(spec[2])
    ...
    for feature, state in grouped.items():
        ...
        certainty = 0.0
        if contributor_rows:
            probs = [1.0 - float(min(1.0, max(0.0, row["value"]))) for row in contributor_rows]
            certainty = 1.0 - float(np.prod([1.0 - p for p in probs], dtype=np.float64))
    ...
    if literals:
        scores = np.asarray([float(beta) * math.log(max(float(row["_certainty"]), eps)) for row in literals], dtype=np.float64)
        scores = scores - scores.max()
        weights = np.exp(scores) / max(float(np.exp(scores).sum()), eps)
...
def prepare_rules_for_export(model, x):
    ...
    for rid in np.flatnonzero(act):
        ...
        activated_rules.append(
            {
                "id": rid_i,
                "condition": rule.get("caption") or _condition_caption(rule.get("specs", ()) or (), model.value_names),
    ...
    *,
    ...
    out = {
        ...
        "combined_condition": " AND ".join(
            f"{name} {op} {_format_explainer_value_for_feature(str(name), _normalise_value(value), getattr(model, 'value_names', None))}"
...
def load_classifier_for_dataset(ds_name: str, algo: str = "RIPPER", model_dir: str | None = None):
    """Helper to load a saved classifier payload (default: ``artifacts/models``)."""
    from DSClassifierMultiQ import DSClassifierMultiQ
    from pathlib import Path
    
    THIS_DIR = Path(__file__).resolve().parent
    if model_dir is None:
        pkl_dir = THIS_DIR.parent / "artifacts" / "models"
    else:
        pkl_dir = Path(model_dir).expanduser().resolve()
